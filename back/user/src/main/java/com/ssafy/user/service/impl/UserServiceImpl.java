...
public class UserServiceImpl implements UserService {
    ...
    public OrderResponseDto createPayment(int userId, CreatePaymentRequestDto requestDto) {
        User user = userMapper.findById(userId);
        return orderClient.createPayment(userId, requestDto.getFundingId(), requestDto.getQuantity(), requestDto.getTotalPrice(), user.getSsafyUserKey(), user.getAccount(), requestDto.getCouponId());
    }
    ...
    public List<Integer> getAgeList(List<GetAgeListRequestDto> dtos) {
        ...
        for (Map<String, Object> row : results) {
            Object groupObj = row.get("ageGroup");
            Object countObj = row.get("count");

            int group = (groupObj instanceof Integer)
                    ? (Integer) groupObj
                    : ((Long) groupObj).intValue();

            int count = (countObj instanceof Integer)
                    ? (Integer) countObj
                    : ((Long) countObj).intValue();
