...
interface ProjectManagementStore {
  ...
  refreshProjects: () => void; // 프로젝트 목록 새로고침 함수 추가
...
const useProjectManagementStore = create<ProjectManagementStore>((set) => ({
  ...
  setOngoingProjects: (projects, currentPage, hasNextPage) =>
    set((state) => ({
      ongoing: {
        ...state.ongoing,
        projects,
  ...
  setFinishedProjects: (projects, currentPage, hasNextPage) =>
    set((state) => ({
      finished: {
        ...state.finished,
        projects,
  ...
  setOngoingLoading: (isLoading) =>
    set((state) => ({
      ongoing: {
        ...state.ongoing,
        isLoading,
        error: isLoading ? null : state.ongoing.error,
      }, // 로딩 시작 시 에러 초기화
    })),

  setFinishedLoading: (isLoading) =>
    set((state) => ({
      finished: {
        ...state.finished,
        isLoading,
        error: isLoading ? null : state.finished.error,
      }, // 로딩 시작 시 에러 초기화
    })),

  setOngoingError: (error) =>
    set((state) => ({
  ...
  goToOngoingPage: (page) =>
    set((state) => ({
      ongoing: { ...state.ongoing, currentPage: page },
    })),

  goToFinishedPage: (page) =>
    set((state) => ({
      finished: { ...state.finished, currentPage: page },
    })),

  // 프로젝트 목록 새로고침 (현재 페이지 유지하면서 로딩 상태만 true로 설정)
  refreshProjects: () =>
    set((state) => ({
      ongoing: { ...state.ongoing, isLoading: true },
      finished: { ...state.finished, isLoading: true },
    })),
