...
import type { ProductDetailContent } from '../api/productService';

// --- 더미 상품 상세 데이터 ---
const dummyProductDetails: ProductDetailContent = {
    fundingId: 999, // 예시 ID
    title: '친환경 대나무 칫솔 (더미 데이터)',
    description: '지구를 생각하는 당신의 선택! 100% 생분해되는 대나무로 만든 칫솔입니다. 플라스틱 사용을 줄이고 지속가능한 생활을 실천하세요. 부드러운 미세모가 잇몸 자극 없이 깨끗하게 닦아줍니다.',
...
export const useProductDetails = (fundingId: number | null) => {
    const [productData, setProductData] = useState<ProductDetailContent | null>(null);
    ...
    useEffect(() => {
        ...
        if (fundingId === null || isNaN(fundingId)) {
            console.log(`[useProductDetails Effect] Invalid fundingId: ${fundingId}. Keeping loading false.`);
        ...
        if (isLoadingRef.current) {
            console.log(`[useProductDetails Effect] Already loading for fundingId: ${fundingId}. Skipping.`);
            return;
        }
        ...
        timerRef.current = setTimeout(() => {
            console.log(`[useProductDetails Effect] setTimeout callback start - fundingId: ${fundingId}`);
            try {
                ...
                console.log(`[useProductDetails Effect] Dummy data loaded (Requested ID: ${fundingId})`);
            } catch (error) {
                setError('데이터 로드 중 오류가 발생했습니다');
                console.error('[useProductDetails Effect] Error loading data:', error);
            } finally {
                // 로딩 상태 종료
                console.log('[useProductDetails Effect] setTimeout callback finish - Setting loading to false.');
                setLoading(false);
                isLoadingRef.current = false;
                timerRef.current = null;
            }
        ...
        return () => {
            ...
            console.log(`[useProductDetails Effect Cleanup] fundingId: ${fundingId}, clearing timer`);
            if (timerRef.current) {
                clearTimeout(timerRef.current);
                timerRef.current = null;
            }
...
const ProductModal: React.FC = () => {
    ...
    const renderContent = () => {
        if (loading) {
            return <div className="spinner-wrapper"><LoadingSpinner message="상품 정보를 불러오는 중입니다..." /></div>;
        }
        ...
        if (!isFullyReady) {
            return <div className="spinner-wrapper"><LoadingSpinner message="데이터 준비 중..." /></div>;
        }
