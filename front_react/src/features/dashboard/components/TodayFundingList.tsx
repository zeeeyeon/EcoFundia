    ...
    return (
        ...
        >
            ...
            <div className="funding-card-body">
                {/* 상품명 및 카테고리 */}
                <div>
                    <span className="funding-card-category">{item.category}</span> {/* 카테고리 표시 추가 */}
                    <h4 className="funding-card-name">{item.productName}</h4>
                </div>
                {/* 진행률 바 및 퍼센트 */}
                <div className="funding-progress-section">
                    {/* 펀딩률 텍스트를 바 위로 이동 */}
                    <span className={`rate-value ${item.fundingRate >= 100 ? 'high-rate' : ''}`}>
                        +{item.fundingRate}%
                    </span>
                ...
                <div className="funding-card-footer">
                    ...
                    <div className="funding-card-amount">
                        <div className="funding-total">{formatCurrency(item.totalAmount)}</div>
                        <div className={`funding-change ${item.changeAmount >= 0 ? 'positive' : 'negative'}`}>
