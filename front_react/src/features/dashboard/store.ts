...
const generateDummyData = () => {
    ...
    const mockProducts: Product[] = [
        ...
        { id: '4', rank: 4, name: '고체 치약 (민트향)', totalFundingAmount: 3100000, fundingRate: 152 },
...
const dashboardStoreCreator: StateCreator<DashboardState> = (set) => ({
    ...
    fetchDashboardData: async () => {
        ...
        if (!tokens) {
            const { mockFundingData, mockAgeGroupData, mockTodayFundingData, mockProducts, mockStats } = generateDummyData();
        ...
        try {
            ...
            if (todayFundingsData.length > 0) {
                todayFundingData = todayFundingsData.map((item, index) => ({
                    ...
                    fundingRate: Math.round((item.currentAmount / (item.currentAmount - item.todayAmount || 1) * 100) || 0)
                }));
            } else {
                console.warn('오늘의 펀딩 데이터가 없습니다. 더미 데이터를 사용합니다.');
            ...
            set({
                ...
                products: mockProducts,
                isLoading: false,
                error: '데이터를 불러오는데 실패했습니다. 개발 모드에서 테스트 데이터를 사용합니다.'
            });
        }
    },
});
